...
const EditAlertForm = () => {
  ...
  useEffect(() => {
    const fetchAlertById = async () => {
      ...
      const jsonResp = await alertResponse.json();
  ...
  const submitForm = async () => {
    ...
    const resp = await makeApiCall(
      `${BASE_URL}${PATH_ALERT}(${formData.AId})`,
      'PATCH',
            JSON.stringify({
        ...otherData,
            AId: parseInt(AId),
    ...
    if (resp.ok) {
      snackbar.enqueueSnackbar('Successfully updated Alert', {
        variant: 'success',
      });
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <div className="action-buttons">
              ...
              <Button
                ...
                variant="contained"
                color="secondary"
                onClick={() => navigate({ pathname: '/alerts' })}
              >
                Cancel
              </Button>
            </div>
