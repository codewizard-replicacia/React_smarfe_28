...
import { alertViewConfig } from "../../utils/display_configuration";
  ...
  useEffect(() => {
    if (id) {
      const fetchAlertById = async () => {
        const AlertResponse = await makeApiCall(
          `${BASE_URL}${PATH_ALERT}(${id})`
        );
        const AlertJsonResp = await AlertResponse.json();
        setAlert(AlertJsonResp);
      };
      fetchAlertById();
    }
  }, [id]);
          ...
          <Grid>
            <Grid item lg={12} xs={12}>
              <Box display="flex" justifyContent="space-between">
                <Typography className="page-heading" variant="h5">View Alert</Typography>
            ...
            <Box marginTop={2} className="form-container">
              <Grid container item lg={12} xs={12}>
                {Object.keys(alertViewConfig).map((config, ind) => (
                  <>
